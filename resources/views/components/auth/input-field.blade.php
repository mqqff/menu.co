...
<div class="flex flex-col gap-1">
    <label for="{{ $name }}" class="text-sm font-semibold text-[#FDE8D0]">
        {{ $label }}
    </label>
    ...
    <div class="relative">
        <input
            ...
            autocomplete="{{ $showToggle ? 'off' : $name }}"
            {{ $autofocus ? 'autofocus' : '' }}
            {{ $required ? 'required' : '' }}

            {{ $attributes->merge([
                'class' => '
                    w-full rounded-full bg-white/95 text-gray-500 placeholder-gray-400
                    text-sm px-5 py-3 outline-none
    ...
    @error($name)
        <p class="text-red-700 font-semibold text-xs mt-1">
            {{ $message }}
        </p>
    @enderror
</div>
